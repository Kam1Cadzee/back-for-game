...
const Content = ({onClose, idWord, onAdd}: IContent) => {
  ...
  return (
    <Search placeholder="input search text" onSearch={handleSearch} allowClear enterButton loading={loading} />
  )
};
...
const AddTranslate = ({idWord, onAdd}:IAddTranslateProps) => {
  ...
  return (
    ...
    >
      <Tooltip title="Add translate">
        <Button size="middle" shape="circle" onClick={() => setIsShow(true)} icon={<PlusOutlined />} />
      </Tooltip>
    </Popover>
  )
};
